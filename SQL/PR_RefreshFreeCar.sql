-- exec pbi.RefreshFreeCars
-- select * from pbi.FreeCars

USE [work]
GO

/****** Object:  StoredProcedure [pbi].[RefreshFreeCars]    Script Date: 10.06.2026 10:33:36 ******/
IF EXISTS (SELECT * FROM sys.procedures WHERE name = 'RefreshFreeCars' AND SCHEMA_NAME(schema_id) = 'pbi')
    DROP PROCEDURE pbi.RefreshFreeCars;
GO
...
Create PROCEDURE [pbi].[RefreshFreeCars] AS
	SET NOCOUNT ON
	...
	IF OBJECT_ID('tempdb..#FreeCar') IS NOT NULL 
		DROP TABLE #FreeCar;

	create table #FreeCar (
	...
	select 
        ...
        q.QuotaIsDone,
		l.GeoZone,
		case when (Remont0+Remont1+Remont2+Remont3+Remont4+Remont5+Remont6+Remont7+Remont8+Remont9) = 0 then 0 else 1 end as CounterRemont, -- в ремонті сьогодні
		case when (Count0+Count1+Count2+Count3+Count4+Count5+Count6+Count7+Count8+Count9) = 0 or 
		(Remont0+Remont1+Remont2+Remont3+Remont4+Remont5+Remont6+Remont7+Remont8+Remont9) <> 0 then 0 else 1 end as CounterWork -- в роботі на сьогодні
		--, R.*
	from #AllCounts C
	...
	DELETE FROM pbi.FreeCars
    INSERT INTO pbi.FreeCars
    SELECT * FROM #FreeCar
