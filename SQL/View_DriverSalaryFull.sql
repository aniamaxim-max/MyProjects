...
cte_RateClasses AS (
    SELECT 
        _InfoRg33137._Fld33138RRef AS Class, 
        _Fld33140 AS DriverSalaryPerDay,
       _InfoRg33137._Fld34210RRef AS Currency,
        ROW_NUMBER() OVER (
            PARTITION BY 
                _InfoRg33137._Fld33138RRef
            ORDER BY _Period DESC
    ...
    FROM _InfoRg33137 -- тариф по класу
	WHERE
		_InfoRg33137._Fld33139RRef = 0x9F700416C8172D6D434B867B43C82D1F AND
		_InfoRg33137._Fld33214RRef = 0x00000000000000000000000000000000
)
...
SELECT
	...
	dt.TaxSum
FROM cte_DriverClasses
inner JOIN _Reference33129 ClassDesc ON ClassDesc._IDRRef = cte_DriverClasses.Class
inner JOIN cte_RateClasses ON cte_RateClasses.Class = cte_DriverClasses.Class
inner JOIN pbi.vb_DimRatesBI dr ON (dr.Dates =  cte_DriverClasses.[Period])
							and (dr.CurrencyRef = cte_RateClasses.Currency)
INNER JOIN pbi.vb_DriverTax dt ON (cte_DriverClasses.Class = 0x924F02B31CC3E40111EFA648B0A9D130 and dt.Num = 2)  
								 or (cte_DriverClasses.Class <> 0x924F02B31CC3E40111EFA648B0A9D130 and dt.Num = 1)

WHERE cte_RateClasses.rn = 1
