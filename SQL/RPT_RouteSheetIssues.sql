...
-- Материализация базового набора (1 раз)
SELECT
    rs.RouteSheetNumber,
    rs.RouteSheetDate,
    rs.TruckRef,
...
-- Индекс для сегмента "Перекриття авто > 2 днів"
...
SELECT
    ...
    CONVERT(VARCHAR(MAX), DriverRef, 2) AS [Водій],
...
SELECT
    RouteSheetNumber,
    LegalNum,
    CONVERT(VARCHAR(MAX), DriverRef, 2),
...
SELECT
    b1.RouteSheetNumber,
    b1.LegalNum,
    CONVERT(VARCHAR(MAX), b1.DriverRef, 2),
